...
- `style.css` — styling
- `canvas-board.js` — 画板全屏绘图模态框（普通 script，同 `market-admin.js` 惯例）：画笔/直线/矩形/椭圆/文字/橡皮 6 工具 + 色板/自定义取色/粗细滑块 + 撤销/重做/清空，重绘栈模型；确定 → 白底合成 PNG（`canvas-{ts}.png`）经 `imageUpload.processFile` 复用聊天附件链路，服务端零改动。入口 ✎ 按钮（`.input-row`，紧邻 `+`）与 `+` 同门联动（`checkKnowledgeUpload` 放行才显示，两按钮均默认 `display:none`）。**画板模态框禁用入场动画**（`.canvas-modal-content { animation: none }`）—— slideUp 位移期间落笔会按事件瞬间坐标记成偏移曲线、橡皮命中检测失准，`CanvasBoardBrowserIT` 锁定该裁定
