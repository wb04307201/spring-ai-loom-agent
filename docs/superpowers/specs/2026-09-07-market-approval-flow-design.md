...
| KB v1 user/public 路由(`/api/knowledge-market` 系,L4492-4574) | 与 v2 重复的 list/pull/submit/withdraw/my-* 腿 | v2 KB router;**注意 app.js 若仍调 `/api/knowledge-market/*` v1 路径,前端同步切 v2**(实施时以 grep app.js 实际调用为准,逐腿核对后再删,防断前端) |
