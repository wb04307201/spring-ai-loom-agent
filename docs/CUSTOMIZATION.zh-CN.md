...
| 项目 | 内容 |
|----------|------------------------------------------------------------------|
| **默认配置** | 库自带 `__init.sql`（基础表结构 + admin seed）和 `__subtask_and_schedule.sql`（子任务 / 定时表 + 侧边栏列）在 `classpath:db/migration/`。应用模块在同目录下加自己的迁移（如 `__xxx.sql`）；Spring Boot 默认 Flyway 按版本号顺序跑、单一 `flyway_schema_history` 表。库**不**覆盖 Flyway 默认路径和 history 表。 |
