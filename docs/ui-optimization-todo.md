...
- [ ] (体验) 子任务 modal **没有"取消全部"按钮**：brief Step 5 期望一键停止当前对话所有运行中的子任务，但目前 toolbar 只有「运行中 N / 已完成 N / 失败 N / 搜 / 过滤 / + 新建 / 收起」，每行要单独点 ■ 取消。schedule modal 倒是有 `cancelAll(convId)` 走 `/spring/ai/loom/schedule/by-conversation/{id}/cancel-all`，可借鉴：① 在 toolbar 加 `[data-cancel-all]` 按钮，调用新接口 `POST /spring/ai/loom/subtask/by-conversation/{id}/cancel-all`，复用 `SubTaskRegistry.killAllByConversation`（已有方法）；② 当 `live.length === 0` 时按钮 disabled；③ UI 文案「全部停止 (N)」
...
- [ ] (视觉一致性) 子任务 modal 颜色方案与 schedule modal **几乎完全一致**（同样 toolbar、同 panel-body、同 composer），但 schedule modal 用 `⏲ ✓ —` 形状区分状态，子任务用 `RUNNING / DONE / FAILED / CANCELLED` 文字+ 颜色 stripe；如果用户并行打开两个 modal，会发现 toolbar 几乎相同但 stripe 配色不一致（子任务 RUNNING = 紫、DONE = 绿、FAILED = 红、CANCELLED = 灰），schedule 没有同色系 stripe。建议：两个 modal 的状态色板统一到 `data-status` attribute + CSS 变量里，避免再次发散
...
- [ ] (可访问性) 子任务 history 的 status label (`RUNNING / DONE / FAILED / CANCELLED`) 颜色承载了全部语义（紫/绿/红/灰），色盲用户无法区分；当前 stripe 也只是颜色块，没有图案/图标区分。建议：① 每个状态加一个 unicode glyph（`▶ ✓ ✗ ◯`），并把状态 chip 的 `::before` 同时显示色 + glyph；② 或保留色但加 `aria-label="状态:已完成"` 等语义
