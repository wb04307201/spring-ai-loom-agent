...
- [ ] (UI 拥挤, 来自 Task 13 截图) skills-market.html 表格第 1 列「名称」只有 132px 宽,而中文名+系统标签+v版本号三行堆叠,实测「测试保存、下载、预览1」会折行成 3 行(行高 101px),视觉上很挤。建议:① 加宽第 1 列到 180-200px,或者把「系统」标签和「v1.0.0」移到第 2 列(描述列),名称列只放名字;② 把「v1.0.0」样式由次要文字改为更小的 chip 样式
- [ ] (UI 拥挤, 来自 Task 13 截图) skills-market.html 「操作」列固定 92px 宽,只能容纳 1 列按钮;对于 PENDING 行(4 个按钮:通过/拒绝/编辑/删除),按钮要 4 行垂直堆叠,管理员需要 4 次精准点击。建议:① 把操作列加宽到 140-160px(横向 2 列排列),或者把"通过/拒绝"用图标+文字(如 ✓/✗),编辑/删除用 icon(✎/🗑),缩小按钮尺寸;② 或者把 PENDING 行的"通过/拒绝"放第 1 行,编辑/删除放第 2 行,用更大的 padding + gap
- [ ] (产品定义歧义, 来自 Task 13 验证) brief 里写「上下架操作」,但当前 `MarketSkill` 只有 PENDING/APPROVED/REJECTED 三态,无独立「上架/下架」状态。"上架"语义由 approve 承担,"下架"语义由 DELETE 承担,导致 brief 的语义需要二次推断。建议:① brief 措辞统一为「审批通过(上架等价物)+ 删除(下架等价物)」;② 或者在 schema 加 `is_published BOOLEAN` 字段,「上架」切换 is_published=true→所有 APPROVED 行出现在 /market-skills 列表,「下架」切 false→管理员可保留数据但用户不可见
- [ ] (可发现性, 来自 Task 13) skills-market.html 「待审批: N」标签放在 toolbar 右侧,与"+新建 Skill"和"刷新"按钮同一行,但视觉上像装饰文本(没有 badge 背景色)。当 N>0 时管理员可能漏看。建议:① 待审批计数加 badge 样式(浅黄背景 + 加粗数字,与 PENDING 行徽章颜色一致),让视觉权重更突出;② 当 N=0 时直接隐藏整段文字(当前实现已隐藏,但应在 0→1 变化时给一个轻微 fade-in 提示)
- [ ] (UX, 来自 Task 13) skills-market.html 编辑/拒绝 modal 标题文案不一致——新建 modal 标题为「新建 Skill(直接 APPROVED)」用括号强调 bypass,但编辑 modal 标题只是「编辑 Skill」,没说编辑会保持当前 status。建议:① 编辑 modal 标题改为「编辑 Skill(保持当前 status)」「编辑 Skill(管理员直接修改)」,让管理员理解 PUT 不会意外修改 status 字段;② 或在 form 底部加一行小字「提示: 编辑不会改变审批状态;如需重新审批请删除后重建」
- [ ] (UX, 来自 Task 13 验证) skills-market.html 删除 modal 提示文案「会同时清理所有 user_skill / role_skill 里对它的引用」对管理员很技术,普通 admin 可能看不懂 user_skill/role_skill 是啥(尤其没读过源码)。建议:① 改为更直白的「删除后:① 所有已拉取此 skill 的用户将失去该 skill;② 所有授予此 skill 的角色将失去该 skill;③ 已审批通过的发布历史将消失」;② 当该 skill 被任何 user_skill / role_skill 引用时,在 modal 顶部加红色横幅提示「⚠️ 当前有 N 个用户 / M 个角色正在使用此 skill」
