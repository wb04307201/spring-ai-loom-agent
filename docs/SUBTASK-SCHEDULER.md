...
- **Persistence**: loom-agent manages its own H2 table `loom_scheduled_task` (now part of the single `V1.0__init.sql` schema; formerly incremental migration `V13`); see "Persistence (Path B — loom-owned)" below. Tasks survive restarts, with `createdAt` preserved so `max-lifetime` accumulates across restarts.
...
- **Table**: `loom_scheduled_task` (now part of the single `V1.0__init.sql` schema; replaced the old `flex_scheduled_task` which is no longer created)
