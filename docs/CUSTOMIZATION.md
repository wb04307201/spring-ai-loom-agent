...
| Item | Details |
|-----------------|---------------------------------------------------------------------------------|
| **Default** | Library ships `__init.sql` (base schema + admin seed) and `__subtask_and_schedule.sql` (sub-task / schedule tables + sidebar columns) at `classpath:db/migration/`. Application modules add their own migration (e.g. `__xxx.sql`) in the same `classpath:db/migration/`; Spring Boot's default Flyway runs them in version order on a single `flyway_schema_history` table. Library does **not** override Flyway's default location or history table. |
...
| 业务模块 SQL | 业务模块作者 seed 自己的 mcp / skill | `src/main/resources/db/migration/V1__xxx.sql`（已被库占用，业务用 V1 或 + 即可） |
