...
public class LoomFileMcpService {
    ...
    @McpTool(name = "read_text_file", description = "读取本地文件内容为文本。path 为相对于基础目录的路径。支持 head（仅前N行）或 tail（仅后N行）参数。")
    ...
    @McpTool(name = "read_media_file", description = "读取本地图片或音频文件，返回 base64 编码数据和 MIME 类型。")
    public String readMediaFile(
            @ToolParam(description = "相对于基础目录的文件路径") String path) {
        return fileOps.readMedia(basePath, path);
    }

    @McpTool(name = "read_multiple_files", description = "同时读取多个文件内容，比逐个读取更高效。单个文件读取失败不会影响其他文件。")
    ...
    @McpTool(name = "write_file", description = "创建新文件或完全覆盖已有文件内容。path 为相对于基础目录的路径。如果文件已存在则覆盖内容。")
    ...
    @McpTool(name = "edit_file", description = "对本地文件进行基于行的编辑，每次编辑用新内容替换精确匹配的文本序列，返回 git 风格的 diff。")
    ...
    @McpTool(name = "create_directory", description = "创建新目录或确保目录已存在，支持创建多级嵌套目录，如果目录已存在则静默成功。")
    ...
    @McpTool(name = "move_file", description = "移动或重命名本地文件。可以跨目录移动或仅重命名。")
    ...
    @McpTool(name = "search_files", description = "在基础目录中递归搜索文件。pattern 为 glob 风格，如 '*.txt' 或 '**/*.java'。不传则返回所有文件。")
    ...
    @McpTool(name = "list_allowed_directories", description = "列出文件操作的基础目录（返回的是**绝对路径**）。写入或创建文件时，path 参数均相对于此目录。")
    public String listAllowedDirectories() {
        return fileOps.listBaseDirectory(basePath);
    }

    @McpTool(name = "list_directory", description = "列出目录内容。区分文件 [FILE] 和目录 [DIR]。支持 depth 参数控制递归深度。")
    ...
    @McpTool(name = "list_directory_with_sizes", description = "列出目录内容及每个项目的大小。区分文件 [FILE] 和目录 [DIR]。")
    public String listDirectoryWithSizes(
            @ToolParam(description = "相对于基础目录的目录路径，空字符串列出根目录") String path) {
        return fileOps.listDirectoryWithSizes(basePath, path);
    }

    @McpTool(name = "directory_tree", description = "获取目录的递归树视图，返回 JSON 格式结构。")
    ...
    @McpTool(name = "get_file_info", description = "获取文件或目录的详细元数据，包括大小、创建时间、修改时间、权限等。")
    ...
    @McpTool(name = "delete_file_or_directory", description = "删除本地文件或目录（递归删除目录与其内容）。必须显式确认（传入 deleteConfirmToken）才能执行。")
