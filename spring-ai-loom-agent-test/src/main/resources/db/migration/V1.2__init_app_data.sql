-- =============================================================
-- V1.2 Skill 演示数据（test 模块 / 应用方 seed,非库层内容）
-- =============================================================
-- 内容:6 个开箱即用的 system skill,seed 到默认 admin 用户 wb04307201 的 user_skill。
-- 分工:库模块 V1.0__init.sql = 建表 + 库层种子(admin 账号 / 2 官方 market_skill /
--   base 角色 + role_skill);应用方 seed 从 V1.1 起(V1.1__mcp_data.sql = MCP 演示数据,
--   本文件 = skill 演示数据)。同一 Flyway 实例按版本顺序执行:V1.0 → V1.1 → V1.2,
--   共用 classpath:db/migration 与 flyway_schema_history。
-- =============================================================


-- =============================================================
-- 6 个开箱即用 skill seed 到默认 admin 用户 wb04307201 的 user_skill
-- （去掉 market_skill admin union view，system skill 不再走"市场 + 特权视图"链路）
-- source='USER_CREATED' / default_loaded=TRUE / locked=FALSE —— 用户可自由修改/删除
-- content 字段直接 hardcode 字符串（不再用 classpath: 占位符，不再依赖 .st 文件）
-- =============================================================

INSERT INTO user_skill (username, name, description, content, source, default_loaded, locked) VALUES
('wb04307201',
 '网络月度事件报告',
...
- 最重要的"红线"：**写 HTML 报告 + 拿预览链接 是这个 skill 唯一的交付物**，
 ...
 'USER_CREATED', TRUE, FALSE),('wb04307201',
 'http测试',
