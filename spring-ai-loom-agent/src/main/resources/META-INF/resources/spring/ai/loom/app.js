...
const knowledge = {
 ...
 switch (status) {
 case 'APPROVED': return { text: '已上架', bg: '#d1fae5', color: '#065f46' };
 ...
 container.innerHTML = '<div style="padding: 40px; text-align: center; color: var(--text-muted);">加载中...</div>';
 ...
 document.querySelectorAll('#ks-sidebar .ks-item').forEach(i => i.classList.remove('selected'));
 ...
 `;
 detail.querySelector('#pull-kb-confirm-btn').addEventListener('click', () => this._pullMarketKnowledge(marketKb.id));
 },
 ...
 // 两段式（点列表项 → 详情面板打开共享表单）
 detail.innerHTML = '<div style="padding: 40px; text-align: center; color: var(--text-muted);"><p style="font-size: 14px; margin-bottom: 8px;">从左侧选一个自建知识库共享到市场</p><p style="font-size: 12px; color: var(--text-muted);">无审批流，提交即上架，<br>其他用户可在「市场」Tab 立即订阅。</p></div>';
 const ownKbs = (this._kbList || []).filter(kb => kb.username === state.username);
 container.innerHTML = '';
 if (ownKbs.length === 0) {
 container.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--text-muted); font-size: 13px;">还没有自建的知识库<br>切到「我的」→ 创建一个再来共享</div>';
 return;
 }
 ...
 `;
 detail.querySelector('#share-kb-confirm-btn').addEventListener('click', () => this._handleShareSubmit(kb));
 },
 ...
 `;
 detail.querySelector('#my-publish-withdraw-btn').addEventListener('click', () => this._withdrawMarketKnowledge(kb.id));
 },
