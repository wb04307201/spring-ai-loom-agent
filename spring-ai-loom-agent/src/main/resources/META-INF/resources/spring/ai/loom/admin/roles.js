(function () {
    ...
    const API = {
        ...
        getKnowledge: (code) => `/spring/ai/loom/admin/roles/${encodeURIComponent(code)}/knowledge`,
        setKnowledge: (code) => `/spring/ai/loom/admin/roles/${encodeURIComponent(code)}/knowledge`,
    ...
    async function openDetail(code) {
        ...
        try {
            const [allRoles, allowed, system, roleSkills, market, roleKnowledge, marketKnowledge] = await Promise.all([
                ...
                loadMarketApproved(),
                (await fetch(API.getKnowledge(code), {credentials: 'include'})).json().catch(() => []),
                loadMarketKnowledge(),
            ]);
            ...
            currentRoleKnowledge = (roleKnowledge || []).map(it => ({marketKnowledgeId: it.marketKnowledgeId, defaultEnabled: it.defaultEnabled !== false}));
    ...
    async function saveDetail() {
        ...
        try {
            ...
            if (r1.ok && r2.ok && r3.ok) {
                showToast(`已保存：${mcpItems.length} 项 MCP + ${skillItems.length} 项技能 + ${knowledgeItems.length} 项知识库`, 'success');
                closeDetail();
                loadRoles();
            } else {
                const t = !r1.ok ? await r1.text() : (!r2.ok ? await r2.text() : await r3.text());
    ...
    function renderRoleSkillList() {
        ...
        if (!currentMarketSkills || currentMarketSkills.length === 0) {
            container.innerHTML = '<div style="color: var(--text-muted); padding: 8px;">市场暂无已审批的技能。请先到<a href="skills-market.html">技能市场</a>创建。</div>';
            return;
        }
    ...
    function renderRoleKnowledgeList() {
        ...
        if (!currentMarketKnowledge || currentMarketKnowledge.length === 0) {
            container.innerHTML = '<div style="color: var(--text-muted); padding: 8px;">市场暂无已审批的知识库。请先到知识库市场创建。</div>';
            return;
        }
        const granted = currentRoleKnowledge
            .map(rk => ({rk, m: currentMarketKnowledge.find(k => k.id === rk.marketKnowledgeId)}))
            .filter(p => p.m);
        const grantedSet = new Set(granted.map(p => p.m.id));
        const available = currentMarketKnowledge.filter(m => !grantedSet.has(m.id));

        const grantedHtml = granted.length === 0
            ? '<div style="color: var(--text-muted); font-size: 12px; padding: 8px;">尚未授权任何知识库。</div>'
            : granted.map(({rk, m}, idx) => {
                const isFirst = idx === 0;
                const isLast = idx === granted.length - 1;
                return `<div style="display: flex; align-items: center; gap: 12px; padding: 10px 12px; border-bottom: 1px solid var(--border-color); background: #f0fdf4;">
                    <span style="width: 24px; text-align: center; color: var(--text-muted); font-size: 12px;">${idx + 1}.</span>
                    <span style="flex: 1;">
                        <div style="font-weight: 500; font-size: 13px;">${escapeHtml(m.name)} <span style="color: var(--text-muted); font-size: 11px;">@${escapeHtml(m.author)}</span></div>
                        <div style="font-size: 11px; color: var(--text-muted);">${escapeHtml((m.description || '').slice(0, 80))}</div>
                    </span>
                    <label style="display: flex; align-items: center; gap: 4px; font-size: 12px; color: var(--text-primary); cursor: pointer; user-select: none;">
        ...
        grantedBox.querySelectorAll('.rk-move-up').forEach(b => b.addEventListener('click', () => moveRoleKnowledge(parseInt(b.getAttribute('data-idx')), -1)));
        grantedBox.querySelectorAll('.rk-move-down').forEach(b => b.addEventListener('click', () => moveRoleKnowledge(parseInt(b.getAttribute('data-idx')), +1)));
        grantedBox.querySelectorAll('.rk-remove').forEach(b => b.addEventListener('click', () => {
            const id = b.getAttribute('data-knowledge-id');
            currentRoleKnowledge = currentRoleKnowledge.filter(it => it.marketKnowledgeId !== id);
            renderRoleKnowledgeList();
        }));
        availBox.querySelectorAll('.rk-add').forEach(b => b.addEventListener('click', () => {
            const id = b.getAttribute('data-knowledge-id');
            if (!roleKnowledgeById(id)) {
                currentRoleKnowledge.push({marketKnowledgeId: id, defaultEnabled: true});
                renderRoleKnowledgeList();
            }
        }));
    }
