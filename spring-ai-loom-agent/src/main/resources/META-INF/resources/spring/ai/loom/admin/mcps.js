(function () {
  ...
  const API = {
    ...
    tools: (name) =>
      `/spring/ai/loom/admin/mcps/tools?name=${encodeURIComponent(name)}`,
  ...
  function renderTable(list) {
    if (!list || list.length === 0) {
      tableContainer.innerHTML =
        '<div class="empty-state">系统未连接任何 MCP 服务</div>';
      return;
    }
    const rows = list
      .map((m) => {
 ...
 <tbody>${rows}</tbody>
    ...
    tableContainer.querySelectorAll(".edit-mcp-btn").forEach((btn) => {
      btn.addEventListener("click", () =>
        openEdit(btn.getAttribute("data-name")),
      );
    });
  }
  ...
  async function openEdit(name) {
    ...
    try {
      ...
      if (m) {
        document.getElementById("edit-mcp-title").textContent =
          `编辑 MCP：${m.name}${m.title ? "（" + m.title + "）" : ""}`;
      ...
      document.getElementById("em-tools").innerHTML =
  ...
  function renderTools(tools) {
    if (!tools || tools.length === 0) {
      document.getElementById("em-tools").innerHTML =
        '<div style="color: var(--text-muted); font-size: 13px; padding: 8px;">此服务无工具</div>';
      return;
    }
    document.getElementById("em-tools").innerHTML = tools
      .map((t) => {
 ...
 style="font-size: 12px;">${escapeHtml(t.description || "")}</textarea>
    ...
    document.querySelectorAll(".delete-tool-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (!ok) return;
        try {
          const r = await fetch(API.updateTool(id), {
            method: "DELETE",
            credentials: "include",
          });
          if (!r.ok) {
  ...
  async function saveEdit() {
    ...
    const toolEls = document.querySelectorAll(
      "#em-tools textarea[data-tool-id]",
    );
    const toolUpdates = [];
    toolEls.forEach((el) => {
