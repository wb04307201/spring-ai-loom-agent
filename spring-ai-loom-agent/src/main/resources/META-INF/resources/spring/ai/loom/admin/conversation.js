(function () {
  ...
  if (username)
    backLink.href = `user.html?username=${encodeURIComponent(username)}`;

  const FLOW_SIZE = 500; // 默认拉 500 条覆盖长对话（之前 200 会截断 282+ event 的对话）

  // B4：按"逻辑时间"分组排序 — SYSTEM → USER → tool → ASSISTANT → 后续
  ...
  function logicalSort(events) {
    ...
    // 修复：第一轮 start = 0（不是 userIdx[0]）—— 第一个 USER 之前的事件（SYSTEM）
    // 不属于任何"轮"，但应该保留在末尾前。
    const userIdx = [];
    byPriority.forEach((e, i) => {
      if (e.type === "USER") userIdx.push(i);
    });
  ...
  async function loadAll() {
    ...
    try {
      const url =
        `/spring/ai/loom/admin/conversations/${encodeURIComponent(conversationId)}/flow?username=${encodeURIComponent(username || "")}&page=0&size=${FLOW_SIZE}` +
        (currentTypes.size > 0
          ? `&types=${Array.from(currentTypes).join(",")}`
          : "");
  ...
  /* ：上下个会话导航 — 拉该用户所有会话，按 updatedAt 排序，
 找当前 conversationId 的上/下一个 conversationId。 */
  ...
  function renderMeta(meta) {
    ...
    const summaryHtml =
      '<div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-color, #e2e8f0); display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; font-size: 12px;">' +
    ...
    metaContent.innerHTML =
      '<table class="conv-meta-table"><tbody>' +
      rows
        .map(
          ([k, v]) =>
            "<tr><th>" +
  ...
  function updateMetaSummary() {
    ...
    if (countEl)
      countEl.textContent = `${total} 条 (用户 ${userMsgs} / 助手 ${asstMsgs})`;
    // 总耗时 = 最后一条事件 - 第一条事件
    ...
    if (allEvents.length >= 2) {
      ...
      if (avgEl)
        avgEl.textContent =
          total > 1 ? `${Math.round(durMs / (total - 1) / 1000)}s` : "-";
    } else {
  ...
  function renderStats(stats) {
    ...
    document.getElementById("stat-subtasks").textContent = (
  ...
  function render() {
    ...
    if (filtered.length === 0) {
      ...
      flowContainer.innerHTML =
        '<div class="empty-state" style="padding: 40px 16px; color: var(--text-muted); font-size: 14px;">' +
  ...
  function highlightMatches(root, keyword) {
    ...
    targets.forEach((text) => {
      const html = text.nodeValue.replace(
        re,
        (m) => `<mark class="flow-search-hit">${m}</mark>`,
      );
  ...
  // 仅保留"全展开 / 全折叠"两个按钮。"紧凑"和"全屏"按钮已移除
  ...
  function renderEvent(ev) {
    ...
    switch (t) {
      case "SYSTEM": {
        // 系统提示词事件 — 展示该对话实际下发的 dynamic system prompt
        icon = "⚙️";
        ...
        body =
          '<div class="flow-meta">长度: ' +
          len +
          " 字符 · 动态拼装（含工具/技能/知识库）</div>" +
          '<details class="flow-system" open><summary>系统提示词</summary>' +
          '<div class="flow-content flow-markdown">' +
          rendered +
          "</div>" +
          "</details>";
        break;
      ...
      case "USER":
        icon = "👤";
        body =
          '<div class="flow-content">' + escapeHtml(d.content || "") + "</div>";
        break;
      case "ASSISTANT": {
        icon = "🤖";
        let thinking = "";
        if (d.thinking) {
          thinking =
            '<details class="flow-thinking"><summary>思考</summary><pre>' +
            escapeHtml(d.thinking) +
            "</pre></details>";
        }
        let metaLine = "";
        if (d.model)
          metaLine +=
            '<div class="flow-meta">model: ' + escapeHtml(d.model) + "</div>";
        if (d.promptTokens != null || d.completionTokens != null) {
          metaLine +=
            '<div class="flow-meta">tokens: ' +
            (d.promptTokens || 0) +
            " + " +
            (d.completionTokens || 0) +
            "</div>";
        }
        // ：用 marked 把 content 渲染成 Markdown（标题/列表/代码块都好看）
        // marked.parse 出来的 HTML 通过 sanitizeHtml 加白名单过滤（防止 XSS）
        let rendered = "";
        if (d.content && window.marked) {
          try {
            const raw = window.marked.parse(d.content, {
              gfm: true,
              breaks: true,
            });
            rendered = window.sanitizeHtml ? window.sanitizeHtml(raw) : raw;
      ...
      case "TOOL_CALL": {
        ...
        if (d.args) {
          ...
          argsHtml =
            '<details class="flow-args"><summary>参数</summary><pre>' +
            escapeHtml(prettyArgs) +
            "</pre></details>";
        }
        body =
          '<div class="flow-event-name">' +
          escapeHtml(d.name || "") +
          "</div>" +
          '<div class="flow-meta">id: ' +
          escapeHtml(d.id || "") +
          "</div>" +
          argsHtml;
        break;
      ...
      case "TOOL_RESULT": {
        ...
        if (d.result) {
          resultHtml =
            '<details class="flow-result"><summary>返回值</summary><pre>' +
            escapeHtml(d.result) +
            "</pre></details>";
        }
        let metaLine = "";
        if (d.durationMs != null)
          metaLine +=
            '<div class="flow-meta">耗时: ' + d.durationMs + " ms</div>";
        if (d.source)
          metaLine +=
            '<div class="flow-meta">来源: ' + escapeHtml(d.source) + "</div>";
        body =
          '<div class="flow-event-name">' +
          escapeHtml(d.name || "") +
          "</div>" +
      ...
      case "SUBTASK": {
        ...
        if (d.status)
          subMeta +=
            '<div class="flow-meta">状态: ' + escapeHtml(d.status) + "</div>";
        if (d.durationMs != null)
          subMeta +=
            '<div class="flow-meta">耗时: ' + d.durationMs + " ms</div>";
        if (d.startedAt)
          subMeta +=
            '<div class="flow-meta">开始: ' + fmtTs(d.startedAt) + "</div>";
        if (d.finishedAt)
          subMeta +=
            '<div class="flow-meta">结束: ' + fmtTs(d.finishedAt) + "</div>";
        let promptHtml = "";
        if (d.prompt)
          promptHtml =
            '<details class="flow-prompt"><summary>prompt</summary><pre>' +
            escapeHtml(d.prompt) +
            "</pre></details>";
        let resultHtml = "";
        if (d.result)
          resultHtml =
            '<details class="flow-result"><summary>result</summary><pre>' +
            escapeHtml(d.result) +
            "</pre></details>";
        let errorHtml = "";
        if (d.error)
          errorHtml =
            '<details class="flow-error"><summary>error</summary><pre>' +
            escapeHtml(d.error) +
            "</pre></details>";
        body =
          '<div class="flow-event-name">' +
      ...
      case "SCHEDULE_FIRE": {
        // 1 个事件 = 1 次 schedule 触发。含 sub-task + execution 全部信息。
        icon = "⏰";
        ...
        if (d.fireTime)
          sMeta +=
            '<div class="flow-meta">触发: ' + fmtTs(d.fireTime) + "</div>";
        if (d.durationMs != null)
          sMeta += '<div class="flow-meta">耗时: ' + d.durationMs + " ms</div>";
        if (d.success === false)
          sMeta +=
            '<div class="flow-meta" style="color:var(--error-color);">⚠ 执行失败</div>';
        if (d.subtaskStatus)
          sMeta +=
            '<div class="flow-meta">子任务: ' +
            escapeHtml(d.subtaskStatus) +
            "</div>";
        if (d.subtaskDurationMs != null && d.subtaskDurationMs > 0)
          sMeta +=
            '<div class="flow-meta">子任务耗时: ' +
            d.subtaskDurationMs +
            " ms</div>";
        let sPrompt = "";
        if (d.prompt)
        ...
        if (d.subtaskResult)
          sResult =
            '<details class="flow-result"><summary>子任务返回值</summary><pre>' +
            escapeHtml(d.subtaskResult) +
            "</pre></details>";
        let sError = "";
        if (d.error)
          sError =
      ...
      case "SCHEDULE": {
        // 纯 SCHEDULE 事件（不带 SUBTASK）— 只在 filter 显示 SUBTASK 不显示 SCHEDULE_FIRE 时出现
        icon = "⏰";
        let sMeta = "";
        if (d.taskName)
          sMeta +=
            '<div class="flow-meta">任务: ' + escapeHtml(d.taskName) + "</div>";
        if (d.fireTime)
          sMeta +=
            '<div class="flow-meta">触发: ' + fmtTs(d.fireTime) + "</div>";
        if (d.durationMs != null)
        ...
        if (d.success === false)
          sMeta +=
            '<div class="flow-meta" style="color:var(--error-color);">失败</div>';
        let sPrompt = "";
        if (d.prompt)
          sPrompt =
            '<details class="flow-prompt"><summary>prompt</summary><pre>' +
    ...
    return (
      '<div class="flow-event flow-event-' +
      t +
      '">' +
  ...
  document
    .querySelectorAll(".conv-filter-label input[type=checkbox]")
    .forEach((cb) => {
      cb.addEventListener("change", () => {
        const t = cb.dataset.type;
        if (cb.checked) currentTypes.add(t);
        else currentTypes.delete(t);
        render();
      });
